...
class ProcessRequest:
    ...
    def process_request(self):
        if nr.sample() < self.kick_probability:
            return "kick"
        if self.queue > 0:
            self.queue -= 1
            self.processed += 1
                ...
                if current_time == block.next:
                    if not isinstance(block, ProcessRequest):
                        ...
                        if next_process == "kick":
                            # if block.name.find("mask_check") != -1:
                            #     refusals["temp_check"] += 1
                            # else:
                            #     refusals[block.name] += 1
                            
                            # if block.name.find("temp_check") != -1:
                            #     refusals["terminal"] += 1
                            # else:
                            #     refusals[block.name] += 1
...
def main():
    ...
    print("┃ Отказы при распределении очереди ┃{:^15d}┃{:^15d}┃".format(min_res["query_window"], max_res["query_window"]))
    ...
    print(
        ...
        f"   максимум - {max_sum};\n"
        "В процентах:\n"
        f"   минимум - {round(min_sum / clients_number * 100, 2)}%;\n"
        f"   максимум - {round(max_sum / clients_number * 100, 2)}%;"
    )
    print()
